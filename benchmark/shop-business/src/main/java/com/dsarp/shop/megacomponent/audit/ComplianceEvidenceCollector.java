...
/**
 * Owns the compliance evidence collector decision within the audit responsibility.
 *
 ...
 */
public final class ComplianceEvidenceCollector extends AbstractBusinessCapability {
    private static final int RESPONSIBILITY_THRESHOLD = 42;
    private static final int RESPONSIBILITY_WEIGHT = 2;

    public ComplianceEvidenceCollector() {
        super("ComplianceEvidenceCollector", RESPONSIBILITY_THRESHOLD, RESPONSIBILITY_WEIGHT);
    }
    ...
    protected int baseScore(OrderContext context) {
        ...
        int attributeAdjustment = context.attribute("audit", "standard").length() % 7;
    ...
    protected List<String> reasons(OrderContext context, int score) {
        ...
        reasons.add("the action has sufficient evidence for an audit trail");
    ...
    public String explain(OrderContext context) {
        ...
        return "compliance evidence collector: " + String.join("; ", decision.reasons());
    }
    ...
    public List<String> requiredEvidence() {
        return List.of("orderId", "customerId", "subtotal", "itemCount", "country", "audit");
    }
    ...
    public String reviewInstruction(OrderContext context) {
        if (canContinueAutomatically(context)) {
            return "continue audit workflow";
        }
        return "review audit evidence for order " + context.orderId();
    }
    ...
    public String trace(OrderContext context) {
        var decision = evaluate(context);
        return context.orderId() + "|ComplianceEvidenceCollector|" + decision.score() + "|" + decision.accepted();
    }
    ...
    public boolean hasExplicitConfiguration(OrderContext context) {
        return !"standard".equals(context.attribute("audit", "standard"));
    }

    /** Exposes a stable capability key for composition and reporting. */
    public String capabilityKey() {
        return "audit.ComplianceEvidenceCollector";
    }
