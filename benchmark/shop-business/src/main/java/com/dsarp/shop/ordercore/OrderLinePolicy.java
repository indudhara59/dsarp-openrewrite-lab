...
public final class OrderLinePolicy {
    private final DiscountPolicy discountPolicy;

    public OrderLinePolicy(DiscountPolicy discountPolicy) {
    ...
    public CapabilityDecision decide(OrderContext context) {
        ...
        int score = Math.max(0, Math.min(100, 100 - payable.remainder(BigDecimal.valueOf(37)).intValue()));
        return new CapabilityDecision("OrderLinePolicy", context.itemCount() > 0, score,
    ...
    public String capabilityKey() {
        return "ordercore.OrderLinePolicy";
    }
