...
/**
 * Owns the feature configuration service decision within the configuration responsibility.
 *
 ...
 */
public final class FeatureConfigurationService extends AbstractBusinessCapability {
    ...
    public FeatureConfigurationService() {
        super("FeatureConfigurationService", RESPONSIBILITY_THRESHOLD, RESPONSIBILITY_WEIGHT);
    }
    ...
    protected int baseScore(OrderContext context) {
        ...
        int attributeAdjustment = context.attribute("configuration", "standard").length() % 7;
    ...
    protected List<String> reasons(OrderContext context, int score) {
        ...
        reasons.add("the configuration policy can make a deterministic decision");
    ...
    public String explain(OrderContext context) {
        ...
        return "feature configuration service: " + String.join("; ", decision.reasons());
    }
    ...
    public List<String> requiredEvidence() {
        return List.of("orderId", "customerId", "subtotal", "itemCount", "country", "configuration");
    }
    ...
    public String reviewInstruction(OrderContext context) {
        if (canContinueAutomatically(context)) {
            return "continue configuration workflow";
        }
        return "review configuration evidence for order " + context.orderId();
    }
    ...
    public String trace(OrderContext context) {
        var decision = evaluate(context);
        return context.orderId() + "|FeatureConfigurationService|" + decision.score() + "|" + decision.accepted();
    }
    ...
    public boolean hasExplicitConfiguration(OrderContext context) {
        return !"standard".equals(context.attribute("configuration", "standard"));
    }

    /** Exposes a stable capability key for composition and reporting. */
    public String capabilityKey() {
        return "configuration.FeatureConfigurationService";
    }
