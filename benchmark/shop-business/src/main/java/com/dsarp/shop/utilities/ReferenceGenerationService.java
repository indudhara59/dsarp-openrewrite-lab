...
/**
 * Owns the reference generation service decision within the utilities responsibility.
 *
 ...
 */
public final class ReferenceGenerationService extends AbstractBusinessCapability {
    private static final int RESPONSIBILITY_THRESHOLD = 45;
    private static final int RESPONSIBILITY_WEIGHT = -5;

    public ReferenceGenerationService() {
        super("ReferenceGenerationService", RESPONSIBILITY_THRESHOLD, RESPONSIBILITY_WEIGHT);
    }
    ...
    protected int baseScore(OrderContext context) {
        ...
        int attributeAdjustment = context.attribute("utilities", "standard").length() % 7;
    ...
    protected List<String> reasons(OrderContext context, int score) {
        ...
        reasons.add("the utilities policy can make a deterministic decision");
    ...
    public String explain(OrderContext context) {
        ...
        return "reference generation service: " + String.join("; ", decision.reasons());
    }
    ...
    public List<String> requiredEvidence() {
        return List.of("orderId", "customerId", "subtotal", "itemCount", "country", "utilities");
    }
    ...
    public String reviewInstruction(OrderContext context) {
        if (canContinueAutomatically(context)) {
            return "continue utilities workflow";
        }
        return "review utilities evidence for order " + context.orderId();
    }
    ...
    public String trace(OrderContext context) {
        var decision = evaluate(context);
        return context.orderId() + "|ReferenceGenerationService|" + decision.score() + "|" + decision.accepted();
    }
    ...
    public boolean hasExplicitConfiguration(OrderContext context) {
        return !"standard".equals(context.attribute("utilities", "standard"));
    }

    /** Exposes a stable capability key for composition and reporting. */
    public String capabilityKey() {
        return "utilities.ReferenceGenerationService";
    }
