...
/**
 * Owns the return resolution service decision within the returns responsibility.
 *
 ...
 */
public final class ReturnResolutionService extends AbstractBusinessCapability {
    private static final int RESPONSIBILITY_THRESHOLD = 41;
    private static final int RESPONSIBILITY_WEIGHT = 2;

    public ReturnResolutionService() {
        super("ReturnResolutionService", RESPONSIBILITY_THRESHOLD, RESPONSIBILITY_WEIGHT);
    }
    ...
    protected int baseScore(OrderContext context) {
        ...
        int attributeAdjustment = context.attribute("returns", "standard").length() % 7;
    ...
    protected List<String> reasons(OrderContext context, int score) {
        ...
        reasons.add("the returns policy can make a deterministic decision");
    ...
    public String explain(OrderContext context) {
        ...
        return "return resolution service: " + String.join("; ", decision.reasons());
    }
    ...
    public List<String> requiredEvidence() {
        return List.of("orderId", "customerId", "subtotal", "itemCount", "country", "returns");
    }
    ...
    public String reviewInstruction(OrderContext context) {
        if (canContinueAutomatically(context)) {
            return "continue returns workflow";
        }
        return "review returns evidence for order " + context.orderId();
    }
    ...
    public String trace(OrderContext context) {
        var decision = evaluate(context);
        return context.orderId() + "|ReturnResolutionService|" + decision.score() + "|" + decision.accepted();
    }
    ...
    public boolean hasExplicitConfiguration(OrderContext context) {
        return !"standard".equals(context.attribute("returns", "standard"));
    }

    /** Exposes a stable capability key for composition and reporting. */
    public String capabilityKey() {
        return "returns.ReturnResolutionService";
    }
