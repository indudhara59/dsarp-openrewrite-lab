...
/**
 * Owns the cart creation service decision within the cart responsibility.
 *
 ...
 */
public final class CartCreationService extends AbstractBusinessCapability {
    ...
    public CartCreationService() {
        super("CartCreationService", RESPONSIBILITY_THRESHOLD, RESPONSIBILITY_WEIGHT);
    }
    ...
    protected int baseScore(OrderContext context) {
        ...
        int attributeAdjustment = context.attribute("cart", "standard").length() % 7;
    ...
    protected List<String> reasons(OrderContext context, int score) {
        ...
        reasons.add("the cart policy can make a deterministic decision");
    ...
    public String explain(OrderContext context) {
        ...
        return "cart creation service: " + String.join("; ", decision.reasons());
    }
    ...
    public List<String> requiredEvidence() {
        return List.of("orderId", "customerId", "subtotal", "itemCount", "country", "cart");
    }
    ...
    public String reviewInstruction(OrderContext context) {
        if (canContinueAutomatically(context)) {
            return "continue cart workflow";
        }
        return "review cart evidence for order " + context.orderId();
    }
    ...
    public String trace(OrderContext context) {
        var decision = evaluate(context);
        return context.orderId() + "|CartCreationService|" + decision.score() + "|" + decision.accepted();
    }
