...
/**
 * Owns the payment retry planner decision within the payment responsibility.
 *
 ...
 */
public final class PaymentRetryPlanner extends AbstractBusinessCapability {
    private static final int RESPONSIBILITY_THRESHOLD = 55;
    private static final int RESPONSIBILITY_WEIGHT = 0;

    public PaymentRetryPlanner() {
        super("PaymentRetryPlanner", RESPONSIBILITY_THRESHOLD, RESPONSIBILITY_WEIGHT);
    }
    ...
    protected int baseScore(OrderContext context) {
        ...
        int attributeAdjustment = context.attribute("payment", "standard").length() % 7;
    ...
    protected List<String> reasons(OrderContext context, int score) {
        ...
        reasons.add("payment processing risk remains acceptable");
    ...
    public String explain(OrderContext context) {
        ...
        return "payment retry planner: " + String.join("; ", decision.reasons());
    }
    ...
    public List<String> requiredEvidence() {
        return List.of("orderId", "customerId", "subtotal", "itemCount", "country", "payment");
    }
    ...
    public String reviewInstruction(OrderContext context) {
        if (canContinueAutomatically(context)) {
            return "continue payment workflow";
        }
        return "review payment evidence for order " + context.orderId();
    }
    ...
    public String trace(OrderContext context) {
        var decision = evaluate(context);
        return context.orderId() + "|PaymentRetryPlanner|" + decision.score() + "|" + decision.accepted();
    }
    ...
    public boolean hasExplicitConfiguration(OrderContext context) {
        return !"standard".equals(context.attribute("payment", "standard"));
    }

    /** Exposes a stable capability key for composition and reporting. */
    public String capabilityKey() {
        return "payment.PaymentRetryPlanner";
    }
