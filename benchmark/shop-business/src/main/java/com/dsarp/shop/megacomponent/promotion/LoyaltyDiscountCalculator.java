...
/**
 * Owns the loyalty discount calculator decision within the promotion responsibility.
 *
 ...
 */
public final class LoyaltyDiscountCalculator extends AbstractBusinessCapability {
    private static final int RESPONSIBILITY_THRESHOLD = 47;
    private static final int RESPONSIBILITY_WEIGHT = -2;

    public LoyaltyDiscountCalculator() {
        super("LoyaltyDiscountCalculator", RESPONSIBILITY_THRESHOLD, RESPONSIBILITY_WEIGHT);
    }
    ...
    protected int baseScore(OrderContext context) {
        ...
        int attributeAdjustment = context.attribute("promotion", "standard").length() % 7;
    ...
    protected List<String> reasons(OrderContext context, int score) {
        ...
        reasons.add("the order is eligible for a bounded commercial incentive");
    ...
    public String explain(OrderContext context) {
        ...
        return "loyalty discount calculator: " + String.join("; ", decision.reasons());
    }
    ...
    public List<String> requiredEvidence() {
        return List.of("orderId", "customerId", "subtotal", "itemCount", "country", "promotion");
    }
    ...
    public String reviewInstruction(OrderContext context) {
        if (canContinueAutomatically(context)) {
            return "continue promotion workflow";
        }
        return "review promotion evidence for order " + context.orderId();
    }
    ...
    public String trace(OrderContext context) {
        var decision = evaluate(context);
        return context.orderId() + "|LoyaltyDiscountCalculator|" + decision.score() + "|" + decision.accepted();
    }
    ...
    public boolean hasExplicitConfiguration(OrderContext context) {
        return !"standard".equals(context.attribute("promotion", "standard"));
    }

    /** Exposes a stable capability key for composition and reporting. */
    public String capabilityKey() {
        return "promotion.LoyaltyDiscountCalculator";
    }
