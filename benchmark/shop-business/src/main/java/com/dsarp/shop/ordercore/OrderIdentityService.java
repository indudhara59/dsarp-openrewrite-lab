...
public final class OrderIdentityService {
    private final DiscountPolicy discountPolicy;

    public OrderIdentityService(DiscountPolicy discountPolicy) {
    ...
    public CapabilityDecision decide(OrderContext context) {
        ...
        int score = Math.max(0, Math.min(100, 100 - payable.remainder(BigDecimal.valueOf(38)).intValue()));
        return new CapabilityDecision("OrderIdentityService", context.itemCount() > 0, score,
    ...
    public String capabilityKey() {
        return "ordercore.OrderIdentityService";
    }
