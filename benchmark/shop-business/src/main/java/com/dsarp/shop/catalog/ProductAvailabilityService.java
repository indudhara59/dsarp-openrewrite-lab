...
/**
 * Owns the product availability service decision within the catalog responsibility.
 *
 ...
 */
public final class ProductAvailabilityService extends AbstractBusinessCapability {
    ...
    public ProductAvailabilityService() {
        super("ProductAvailabilityService", RESPONSIBILITY_THRESHOLD, RESPONSIBILITY_WEIGHT);
    }
    ...
    protected int baseScore(OrderContext context) {
        ...
        int attributeAdjustment = context.attribute("catalog", "standard").length() % 7;
    ...
    protected List<String> reasons(OrderContext context, int score) {
        ...
        reasons.add("the catalog policy can make a deterministic decision");
    ...
    public String explain(OrderContext context) {
        ...
        return "product availability service: " + String.join("; ", decision.reasons());
    }
    ...
    public List<String> requiredEvidence() {
        return List.of("orderId", "customerId", "subtotal", "itemCount", "country", "catalog");
    }
    ...
    public String reviewInstruction(OrderContext context) {
        if (canContinueAutomatically(context)) {
            return "continue catalog workflow";
        }
        return "review catalog evidence for order " + context.orderId();
    }
    ...
    public String trace(OrderContext context) {
        var decision = evaluate(context);
        return context.orderId() + "|ProductAvailabilityService|" + decision.score() + "|" + decision.accepted();
    }
    ...
    public boolean hasExplicitConfiguration(OrderContext context) {
        return !"standard".equals(context.attribute("catalog", "standard"));
    }

    /** Exposes a stable capability key for composition and reporting. */
    public String capabilityKey() {
        return "catalog.ProductAvailabilityService";
    }
