...
/**
 * Owns the delivery constraint validator decision within the validation responsibility.
 *
 ...
 */
public final class DeliveryConstraintValidator extends AbstractBusinessCapability {
    private static final int RESPONSIBILITY_THRESHOLD = 47;
    private static final int RESPONSIBILITY_WEIGHT = 1;

    public DeliveryConstraintValidator() {
        super("DeliveryConstraintValidator", RESPONSIBILITY_THRESHOLD, RESPONSIBILITY_WEIGHT);
    }
    ...
    protected int baseScore(OrderContext context) {
        ...
        int attributeAdjustment = context.attribute("validation", "standard").length() % 7;
    ...
    protected List<String> reasons(OrderContext context, int score) {
        ...
        reasons.add("order facts are complete and consistent");
    ...
    public String explain(OrderContext context) {
        ...
        return "delivery constraint validator: " + String.join("; ", decision.reasons());
    }
    ...
    public List<String> requiredEvidence() {
        return List.of("orderId", "customerId", "subtotal", "itemCount", "country", "validation");
    }
    ...
    public String reviewInstruction(OrderContext context) {
        if (canContinueAutomatically(context)) {
            return "continue validation workflow";
        }
        return "review validation evidence for order " + context.orderId();
    }
    ...
    public String trace(OrderContext context) {
        var decision = evaluate(context);
        return context.orderId() + "|DeliveryConstraintValidator|" + decision.score() + "|" + decision.accepted();
    }
    ...
    public boolean hasExplicitConfiguration(OrderContext context) {
        return !"standard".equals(context.attribute("validation", "standard"));
    }

    /** Exposes a stable capability key for composition and reporting. */
    public String capabilityKey() {
        return "validation.DeliveryConstraintValidator";
    }
