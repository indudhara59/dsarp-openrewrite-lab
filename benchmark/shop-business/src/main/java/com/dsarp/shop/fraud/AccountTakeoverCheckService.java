...
/**
 * Owns the account takeover check service decision within the fraud responsibility.
 *
 ...
 */
public final class AccountTakeoverCheckService extends AbstractBusinessCapability {
    private static final int RESPONSIBILITY_THRESHOLD = 55;
    private static final int RESPONSIBILITY_WEIGHT = -2;

    public AccountTakeoverCheckService() {
        super("AccountTakeoverCheckService", RESPONSIBILITY_THRESHOLD, RESPONSIBILITY_WEIGHT);
    }
    ...
    protected int baseScore(OrderContext context) {
        ...
        int attributeAdjustment = context.attribute("fraud", "standard").length() % 7;
    ...
    protected List<String> reasons(OrderContext context, int score) {
        ...
        reasons.add("the fraud policy can make a deterministic decision");
    ...
    public String explain(OrderContext context) {
        ...
        return "account takeover check service: " + String.join("; ", decision.reasons());
    }
    ...
    public List<String> requiredEvidence() {
        return List.of("orderId", "customerId", "subtotal", "itemCount", "country", "fraud");
    }
    ...
    public String reviewInstruction(OrderContext context) {
        if (canContinueAutomatically(context)) {
            return "continue fraud workflow";
        }
        return "review fraud evidence for order " + context.orderId();
    }
    ...
    public String trace(OrderContext context) {
        var decision = evaluate(context);
        return context.orderId() + "|AccountTakeoverCheckService|" + decision.score() + "|" + decision.accepted();
    }
    ...
    public boolean hasExplicitConfiguration(OrderContext context) {
        return !"standard".equals(context.attribute("fraud", "standard"));
    }

    /** Exposes a stable capability key for composition and reporting. */
    public String capabilityKey() {
        return "fraud.AccountTakeoverCheckService";
    }
