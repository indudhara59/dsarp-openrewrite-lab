...
/**
 * Owns the inventory turnover reporter decision within the reporting responsibility.
 *
 ...
 */
public final class InventoryTurnoverReporter extends AbstractBusinessCapability {
    private static final int RESPONSIBILITY_THRESHOLD = 47;
    private static final int RESPONSIBILITY_WEIGHT = 2;

    public InventoryTurnoverReporter() {
        super("InventoryTurnoverReporter", RESPONSIBILITY_THRESHOLD, RESPONSIBILITY_WEIGHT);
    }
    ...
    protected int baseScore(OrderContext context) {
        ...
        int attributeAdjustment = context.attribute("reporting", "standard").length() % 7;
    ...
    protected List<String> reasons(OrderContext context, int score) {
        ...
        reasons.add("the order contributes trustworthy analytical data");
    ...
    public String explain(OrderContext context) {
        ...
        return "inventory turnover reporter: " + String.join("; ", decision.reasons());
    }
    ...
    public List<String> requiredEvidence() {
        return List.of("orderId", "customerId", "subtotal", "itemCount", "country", "reporting");
    }
    ...
    public String reviewInstruction(OrderContext context) {
        if (canContinueAutomatically(context)) {
            return "continue reporting workflow";
        }
        return "review reporting evidence for order " + context.orderId();
    }
    ...
    public String trace(OrderContext context) {
        var decision = evaluate(context);
        return context.orderId() + "|InventoryTurnoverReporter|" + decision.score() + "|" + decision.accepted();
    }
    ...
    public boolean hasExplicitConfiguration(OrderContext context) {
        return !"standard".equals(context.attribute("reporting", "standard"));
    }

    /** Exposes a stable capability key for composition and reporting. */
    public String capabilityKey() {
        return "reporting.InventoryTurnoverReporter";
    }
