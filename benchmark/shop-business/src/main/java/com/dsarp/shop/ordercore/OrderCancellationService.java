...
public final class OrderCancellationService {
    ...
    public OrderCancellationService(DiscountPolicy discountPolicy) {
    ...
    public CapabilityDecision decide(OrderContext context) {
        ...
        int score = Math.max(0, Math.min(100, 100 - payable.remainder(BigDecimal.valueOf(35)).intValue()));
        return new CapabilityDecision("OrderCancellationService", context.itemCount() > 0, score,
    ...
    public String capabilityKey() {
        return "ordercore.OrderCancellationService";
    }
