...
class PaymentCaptureServiceTest {
    ...
    void producesExplainableDeterministicDecision() {
        OrderContext context = new OrderContext("O-10", "C-10", new BigDecimal("110.50"),
                1, "DE", Instant.parse("2026-01-15T10:00:00Z"), Map.of());
        PaymentCaptureService service = new PaymentCaptureService();
        CapabilityDecision first = service.evaluate(context);
        CapabilityDecision second = service.evaluate(context);
        assertThat(first).isEqualTo(second);
        assertThat(first.capability()).isEqualTo("PaymentCaptureService");
