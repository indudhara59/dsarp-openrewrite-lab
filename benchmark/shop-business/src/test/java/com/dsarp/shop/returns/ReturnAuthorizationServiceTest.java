...
class ReturnAuthorizationServiceTest {
    ...
    void producesExplainableDeterministicDecision() {
        OrderContext context = new OrderContext("O-71", "C-71", new BigDecimal("171.50"),
                2, "DE", Instant.parse("2026-01-15T10:00:00Z"), Map.of());
        ReturnAuthorizationService service = new ReturnAuthorizationService();
        CapabilityDecision first = service.evaluate(context);
        CapabilityDecision second = service.evaluate(context);
        assertThat(first).isEqualTo(second);
        assertThat(first.capability()).isEqualTo("ReturnAuthorizationService");
