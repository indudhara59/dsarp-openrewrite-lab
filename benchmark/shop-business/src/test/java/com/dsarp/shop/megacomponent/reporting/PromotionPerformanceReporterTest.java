...
class PromotionPerformanceReporterTest {
    @Test
    void producesExplainableDeterministicDecision() {
        OrderContext context = new OrderContext("O-32", "C-32", new BigDecimal("132.50"),
                3, "DE", Instant.parse("2026-01-15T10:00:00Z"), Map.of());
        PromotionPerformanceReporter service = new PromotionPerformanceReporter();
        CapabilityDecision first = service.evaluate(context);
        CapabilityDecision second = service.evaluate(context);
        assertThat(first).isEqualTo(second);
        assertThat(first.capability()).isEqualTo("PromotionPerformanceReporter");
