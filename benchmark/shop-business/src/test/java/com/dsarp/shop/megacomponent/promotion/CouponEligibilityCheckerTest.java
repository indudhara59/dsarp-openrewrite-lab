...
class CouponEligibilityCheckerTest {
    @Test
    void producesExplainableDeterministicDecision() {
        OrderContext context = new OrderContext("O-44", "C-44", new BigDecimal("144.50"),
                5, "DE", Instant.parse("2026-01-15T10:00:00Z"), Map.of());
        CouponEligibilityChecker service = new CouponEligibilityChecker();
        CapabilityDecision first = service.evaluate(context);
        CapabilityDecision second = service.evaluate(context);
        assertThat(first).isEqualTo(second);
        assertThat(first.capability()).isEqualTo("CouponEligibilityChecker");
