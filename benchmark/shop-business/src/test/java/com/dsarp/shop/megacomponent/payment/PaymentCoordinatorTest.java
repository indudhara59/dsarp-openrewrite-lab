...
class PaymentCoordinatorTest {
    ...
    void producesExplainableDeterministicDecision() {
        OrderContext context = new OrderContext("O-8", "C-8", new BigDecimal("108.50"),
                4, "DE", Instant.parse("2026-01-15T10:00:00Z"), Map.of());
        PaymentCoordinator service = new PaymentCoordinator();
        CapabilityDecision first = service.evaluate(context);
        CapabilityDecision second = service.evaluate(context);
        assertThat(first).isEqualTo(second);
        assertThat(first.capability()).isEqualTo("PaymentCoordinator");
