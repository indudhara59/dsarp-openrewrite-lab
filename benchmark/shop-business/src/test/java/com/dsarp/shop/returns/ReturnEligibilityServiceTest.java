...
class ReturnEligibilityServiceTest {
    ...
    void producesExplainableDeterministicDecision() {
        OrderContext context = new OrderContext("O-70", "C-70", new BigDecimal("170.50"),
                1, "DE", Instant.parse("2026-01-15T10:00:00Z"), Map.of());
        ReturnEligibilityService service = new ReturnEligibilityService();
        CapabilityDecision first = service.evaluate(context);
        CapabilityDecision second = service.evaluate(context);
        assertThat(first).isEqualTo(second);
        assertThat(first.capability()).isEqualTo("ReturnEligibilityService");
