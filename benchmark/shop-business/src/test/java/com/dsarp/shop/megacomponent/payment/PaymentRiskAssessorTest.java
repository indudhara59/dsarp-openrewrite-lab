...
class PaymentRiskAssessorTest {
    @Test
    void producesExplainableDeterministicDecision() {
        OrderContext context = new OrderContext("O-12", "C-12", new BigDecimal("112.50"),
                3, "DE", Instant.parse("2026-01-15T10:00:00Z"), Map.of());
        PaymentRiskAssessor service = new PaymentRiskAssessor();
        CapabilityDecision first = service.evaluate(context);
        CapabilityDecision second = service.evaluate(context);
        assertThat(first).isEqualTo(second);
        assertThat(first.capability()).isEqualTo("PaymentRiskAssessor");
