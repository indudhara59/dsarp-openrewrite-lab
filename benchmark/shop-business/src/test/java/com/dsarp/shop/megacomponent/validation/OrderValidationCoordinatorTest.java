...
class OrderValidationCoordinatorTest {
    @Test
    void producesExplainableDeterministicDecision() {
        OrderContext context = new OrderContext("O-1", "C-1", new BigDecimal("101.50"),
                2, "DE", Instant.parse("2026-01-15T10:00:00Z"), Map.of());
        OrderValidationCoordinator service = new OrderValidationCoordinator();
        CapabilityDecision first = service.evaluate(context);
        CapabilityDecision second = service.evaluate(context);
        assertThat(first).isEqualTo(second);
        assertThat(first.capability()).isEqualTo("OrderValidationCoordinator");
