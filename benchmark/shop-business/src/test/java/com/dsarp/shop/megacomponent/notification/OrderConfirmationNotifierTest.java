...
class OrderConfirmationNotifierTest {
    ...
    void producesExplainableDeterministicDecision() {
        OrderContext context = new OrderContext("O-23", "C-23", new BigDecimal("123.50"),
                4, "DE", Instant.parse("2026-01-15T10:00:00Z"), Map.of());
        OrderConfirmationNotifier service = new OrderConfirmationNotifier();
        CapabilityDecision first = service.evaluate(context);
        CapabilityDecision second = service.evaluate(context);
        assertThat(first).isEqualTo(second);
        assertThat(first.capability()).isEqualTo("OrderConfirmationNotifier");
