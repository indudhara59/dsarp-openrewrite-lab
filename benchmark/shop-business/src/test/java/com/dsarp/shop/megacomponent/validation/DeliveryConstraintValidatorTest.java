...
class DeliveryConstraintValidatorTest {
    @Test
    void producesExplainableDeterministicDecision() {
        OrderContext context = new OrderContext("O-5", "C-5", new BigDecimal("105.50"),
                1, "DE", Instant.parse("2026-01-15T10:00:00Z"), Map.of());
        DeliveryConstraintValidator service = new DeliveryConstraintValidator();
        CapabilityDecision first = service.evaluate(context);
        CapabilityDecision second = service.evaluate(context);
        assertThat(first).isEqualTo(second);
        assertThat(first.capability()).isEqualTo("DeliveryConstraintValidator");
