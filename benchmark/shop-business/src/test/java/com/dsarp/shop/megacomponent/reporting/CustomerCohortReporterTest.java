...
class CustomerCohortReporterTest {
    @Test
    void producesExplainableDeterministicDecision() {
        OrderContext context = new OrderContext("O-34", "C-34", new BigDecimal("134.50"),
                5, "DE", Instant.parse("2026-01-15T10:00:00Z"), Map.of());
        CustomerCohortReporter service = new CustomerCohortReporter();
        CapabilityDecision first = service.evaluate(context);
        CapabilityDecision second = service.evaluate(context);
        assertThat(first).isEqualTo(second);
        assertThat(first.capability()).isEqualTo("CustomerCohortReporter");
